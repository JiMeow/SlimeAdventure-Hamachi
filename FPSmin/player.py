...
class Player(pygame.sprite.Sprite):
    ...
    def init_player_image(self, color, name, x, y):
        ...
        self.original_image = pygame.image.load(
            f"FPSmin/assets/{color}.png").convert()
    ...
    def mouse(self):
        ...
        self.face_direction = pygame.math.Vector2(
            mouse[0] - self.rect.centerx, mouse[1] - self.rect.centery)
        if self.face_direction.magnitude() != 0:
            self.face_direction.normalize_ip()
        if pygame.mouse.get_pressed()[0]:
            if self.face_direction.magnitude() != 0:
                bullet = {"pos": [self.rect.centerx, self.rect.centery], "direction": [
                    self.face_direction.x, self.face_direction.y]}
    ...
    def move(self, dt):
        self.move_direction = pygame.math.Vector2(
            self.target_pos[0] - self.rect.centerx, self.target_pos[1] - self.rect.centery)
