...
class Game:
    def __init__(self):
        ...
        self.player = Player(
            self.init_pos[0], self.init_pos[1], "green", control=True)
        self.id = network.id
        self.players = {}
    ...
    def run(self):
        while self.running:
                ...
                if k in self.players:
                    ...
                    if v["event"]:
                        face_direction = pygame.math.Vector2(
                            v["event"]["direction"][0], v["event"]["direction"][1])
                        Projectile(v["event"]["pos"][0], v["event"]
                                   ["pos"][1], face_direction)
                else:
            ...
            debug(
                f"move_durection: {self.player.move_direction.x:.2f},{self.player.move_direction.y:.2f}", 0, 10)
            debug(f"pos: {self.player.rect.x},{self.player.rect.y}", 0, 20)
            debug(
                f"face_direction: {self.player.face_direction.x},{self.player.face_direction.y}", 0, 30)
